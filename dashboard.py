...
# ==========================================
# 2. MODEL LOADER (FIXED FOR UNBOUNDLOCALERROR)
# ==========================================
@st.cache_resource
def load_resources():
    """
    Loads the Champion EBMs and their Forensic Audit Scores.
    Initializes variables to None to prevent scope errors on failure.
    """
    # 🟢 FIX: Pre-define variables as None
    k_struct, s_struct = None, None
    k_stats, s_stats = None, None
    ...
    try:
        ...
        # Now k_stats and s_stats are defined as None, so this return works!
...
if k_struct is None:
    st.warning("⚠️ Model files not found. Please ensure .pkl and .json files are in the root directory.")
    st.stop()
...
with tab3:
    ...
    # Select the correct stats object loaded from load_resources()
    current_stats = k_stats if is_kembung else s_stats
    
    if current_stats is None:
        st.error(f"⚠️ Performance Metrics JSON missing for {species_choice}. Please upload kembung_metrics.json or selar_metrics.json.")
    else:
        # 2. KEY PERFORMANCE INDICATORS (Using your new JSON keys)
        c1, c2, c3 = st.columns(3)
        with c1:
            # Displays the "95.39%" string
            st.metric("Overall Accuracy", current_stats.get("Accuracy", "N/A"), 
                      help="100% - MAPE (From Dynamic Blind Test)")
        with c2:
            # Displays the "4.61%" string
            st.metric("Forensic Error (MAPE)", current_stats.get("MAPE", "N/A"), 
                      help="Mean Absolute Percentage Error", delta_color="inverse")
        with c3:
            # Displays the "RM 0.8298" string
            st.metric("Avg Price Deviation", current_stats.get("Structural_MAE", "N/A"), 
                      help="Mean Absolute Error over 8 Weeks")
    ...
    # Helper function for Image Injection
    ...
    with h_col1:
        st.markdown("**Test A: Predicting one week at a time using currently known market conditions**")
        if b64_struct:
            ...
            st.info("✅ **Structural Consistency:** The flat RMSE line confirms the model is mathematically stable. Accuracy remains high as long as economic inputs are precise.")
        else:
            st.warning(f"Structural plot missing: {file_struct}") 

    with h_col2:
        st.markdown("**Test B: Predicting to any selected week using projected market conditions**") 
        if b64_blind:
            ...
            st.info(f"🕶️ **Dynamic Blind Test:** Demonstrates error growth over 8 weeks when the system must forecast its own inputs. Valid within a 4-week window at ± {error_margin}.")
        else:
    ...
    with rec_col2:
        st.info("**Operational Protocol:** Policymakers should rely on the 1-4 week forecast horizon. Predictions beyond 30 days are advisory only, as error margins begin to compound due to global economic shifts.")
